...
def solve_kinematic_bicycle(x0, delta_rad, v_mps):
    ...
    for i in range(1, p1.shape[0]):
        ...
        #  Start of your code  #
        ########################
        beta = math.atan(math.tan(delta_rad[i - 1] * lr_m / (lf_m + lr_m)))
        p1_d = v_mps[i - 1] * math.cos(psi[i - 1] + beta)
        p2_d = v_mps[i - 1] * math.sin(psi[i - 1] + beta)
        psi_d = v_mps[i - 1] * math.tan(delta_rad[i - 1]) / (lf_m + lr_m)
        p1[i] = p1[i - 1] + tS * p1_d
        p2[i] = p2[i - 1] + tS * p2_d
        psi[i] = psi[i - 1] + tS * psi_d
        continue
