...
def create_train_txt(dataset_path):
    ...
    dirs = os.listdir(os.path.join(abs_path, "images/train"))
    for pic in dirs:
        img_names.append(pic)
    f = open(os.path.join(abs_path, "train.txt"), "w")
    for number in range(1, len(img_names)):
        f.write(img_names[number - 1] + "\n")
    ########################
    #   End of your code   #
    ########################
