...
def check_trajectory(ttc, a_comb, bound_collision, ttc_treshold=2.0, a_comb_threshold=15.0):
    ...
    #  Start of your code  #
    ########################
    if (ttc > ttc_treshold) & (bound_collision is False):
        safe = True
        for element in a_comb:
            if element > a_comb_threshold:
                safe = False
    else:
        safe = False
    ########################
...
def check_bound_collision(path, bound, veh_width=2.8):
    ...
    path_buffer = path_line.buffer(veh_width)
    intersection = bound_line.intersects(path_buffer)
