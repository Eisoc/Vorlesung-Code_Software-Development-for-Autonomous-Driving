...
def get_aabb_polygon(trajectory: dict):
    ...
    union = unary_union(trajectory['vehicle_polygons'])
    aabb = union.convex_hull.envelope
    ########################
    #   End of your code   #
    ########################
...
def calculate_mbc_radius(rectangle_width: float, rectangle_length: float):
    ...
    #  Start of your code  #
    ########################
    radius = ((0.5 * rectangle_width) ** 2 + ((0.5 * rectangle_length) ** 2)) ** 0.5
    ########################
    #   End of your code   #
    ########################
...
def check_circle_collision(xy1: np.ndarray, xy2: np.ndarray, r: float):
    ...
    #  Start of your code  #
    ########################
    d_circle = ((xy2[0] - xy1[0]) ** 2 + (xy2[1] - xy1[1]) ** 2) ** 0.5
    if d_circle <= 2 * r:
...
def polygon_collision_check(trajectory1: dict, trajectory2: dict):
    ...
    for i_check in range(len(trajectory1["vehicle_polygons"])):
        check = trajectory1['vehicle_polygons'][i_check].intersects(trajectory2['vehicle_polygons'][i_check])
        if check is True:
...
def mbc_collision_check(trajectory1: dict, trajectory2: dict, mbc_radius: float,
    ...
    ########################
    mbc_collision = False
    for i_check in range(len(trajectory1["xy"])):
        check = check_circle_collision(trajectory1["xy"][i_check], trajectory2["xy"][i_check], mbc_radius)
        if check is True:
            mbc_collision = True
    ########################
    #   End of your code   #
