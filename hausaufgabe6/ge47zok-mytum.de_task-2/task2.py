...
def dijkstra(nodes, neighbors_list, start_node, end_node):
    ...
    distance_from_start[start_node] = 0.0
    for node_other in nodes:
        if node_other != start_node:
            distance_from_start[node_other] = inf
    ########################
    #   End of your code   #
    ########################
    ...
    while unvisited_nodes:
        ...
        #  Start of your code  #
        ########################
        for node_vor in neighbors_list[current_node]:
            new_path = distance_from_start[current_node] + node_vor[1]
            if node_vor[0] in unvisited_nodes:
                if new_path < distance_from_start[node_vor[0]]:
                    distance_from_start[node_vor[0]] = new_path
                    previous_node[node_vor[0]] = current_node
        ########################
        #   End of your code   #
        ########################
    ...
    ########################
    c_node = end_node
    path.appendleft(c_node)
    while previous_node[c_node] != start_node:
        path.appendleft(previous_node[c_node])
        c_node = previous_node[c_node]
    if previous_node[c_node] == start_node:
